...
- Images (in-content): `max-height: min(50vh, 400px)`
- Full-bleed slide backgrounds: must cover the entire slide — no letterboxing. Use `background-size: cover` and `background-position: center`, or a full-bleed `<img>` with `object-fit: cover` and inset/absolute fill to match the slide bounds
