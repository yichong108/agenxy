...
const DOCKER_COMPOSE_FILE = 'docker-compose.langfuse.yml'
...
function cmdStart() {
  ...
  const root = getProjectRoot()
  const composeFile = resolve(root, DOCKER_COMPOSE_FILE)
  ...
  const envArgs = existsSync(envFile) ? `--env-file ${ENV_LOCAL}` : ''

  log('📦 拉取镜像并启动服务（首次启动可能需要几分钟）...', 'cyan')

  try {
    run(`docker-compose -f ${DOCKER_COMPOSE_FILE} ${envArgs} up -d`)
...
function cmdStop() {
  ...
  try {
    run(`docker-compose -f ${DOCKER_COMPOSE_FILE} ${envArgs} stop`)
    log('✅ 服务已停止', 'green')
  } catch {
...
function cmdStatus() {
  ...
  try {
    const output = run(`docker-compose -f ${DOCKER_COMPOSE_FILE} ${envArgs} ps`, { silent: true })
    if (output.trim()) {
      console.log(output)
    } else {
      log('ℹ️  没有运行中的服务', 'dim')
...
function cmdLogs() {
  ...
  try {
    run(`docker-compose -f ${DOCKER_COMPOSE_FILE} ${envArgs} logs -f`, { ignoreError: true })
  } catch {
    // 用户按 Ctrl+C 退出
  }
}
...
async function cmdReset() {
  ...
  try {
    log('🗑️  正在删除数据...', 'yellow')
    run(`docker-compose -f ${DOCKER_COMPOSE_FILE} ${envArgs} down -v`)
    log('✅ 数据已清除', 'green')
